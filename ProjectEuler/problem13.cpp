#include <bits/stdc++.h>
using namespace std;
int a[][50]={{3, 7, 1, 0, 7, 2, 8, 7, 5, 3, 3, 9, 0, 2, 1, 0, 2, 7, 9, 8, 7, 9, 7, 9, 9, 8, 2, 2, 0, 8, 3, 7, 5, 9, 0, 2, 4, 6, 5, 1, 0, 1, 3, 5, 7, 4, 0, 2, 5, 0},
...
void add_carry(vector<int>& colSums){
    for(int col=49;col>=1;col--){
        int carry=colSums[col]/10;
        colSums[col]-=(10*carry);
        colSums[col-1]+=carry;
    }
    // auto first=colSums.begin();
    // while(colSums[distance(colSums.begin(),*first)]/10!=0){
    //     colSums.insert(colSums.begin(),colSums[distance(colSums.begin(),*first)]%10);
    //     colSums[distance(colSums.begin(),*first)]/=10;
    // }
    //
}
...
void printVector(vector<int> a, int start=0, int end=0){
    if(end==0)
        end=a.size();
...
int main(){
    ...
    reverse(colSums.begin(),colSums.end());
    printVector(colSums);
    add_carry(colSums);
    // cout << "\n\n----------------------------------------------\n\n";
    printVector(colSums,0,8);
    // cout << get_num() << endl;
    return 0;
}
